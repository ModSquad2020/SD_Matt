class CreateWindows:
    ...
    def __init__(self, secondaryStructure='(((((((,,<<<<___.___.__>>>>,<<<<<_______>>>>>,,...........,,,<<<<<_______>>>>>))))))):', sequence='', desiredRegions=['AAStem','DArm','AnticodonLoop', 'TArm', 'Introns']):
    ...
    def populateDictionary(self):
        ...
        for n, pattern in enumerate(patterns):
            ...
            for matchNum, match in enumerate(matches):
                ...
                if n==0 or n==1:
                    alignedRegions['AAStem'].append( match.span() )
                elif n==2:
                    if matchNum==0:
                        alignedRegions['DArm'].append( match.span() )
                    elif matchNum==1:
                        alignedRegions['AnticodonLoop'].append( match.span() )
                    else:
                        alignedRegions['TArm'].append( match.span() )
                else:
